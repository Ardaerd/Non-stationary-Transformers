...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # Weights for ARIMA and Transformer outputs
        self.arima_weight = nn.Parameter(torch.tensor(0.3))
        self.transformer_weight = nn.Parameter(torch.tensor(0.7))

        # Enhanced FC layer with more capacity
        fc_layers = [
            nn.Linear(configs.c_out, 512), nn.ReLU(),
    ...
    def fit_arima(self, series, order):
    ...
    def forecast_arima(self, model, steps):
        ...
        return forecast  # Only the forecast values
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, enc_self_mask=None, dec_self_mask=None, dec_enc_mask=None):
        ...
        arima_forecasts = []
        arima_order = (1, 0, 1)  # Define your ARIMA order here (p, d, q)

        for i in range(batch_size):
            series = x_raw[i, :, 0].cpu().numpy()  # Assuming single feature for ARIMA
            arima_model = self.fit_arima(series, arima_order)
            arima_forecast = self.forecast_arima(arima_model, self.pred_len)
            arima_forecasts.append(arima_forecast)

        arima_forecasts = torch.tensor(arima_forecasts).to(device)  # Convert to tensor
        arima_forecasts = arima_forecasts.unsqueeze(-1).expand(batch_size, self.pred_len, x_enc.size(2))  # Expand to match feature dimension
        ...
        dec_out_selected = dec_out[:, -self.pred_len:, :].float()
        arima_forecasts = arima_forecasts.float()

        # Concatenate along the feature dimension
        combined_forecasts = self.arima_weight * arima_forecasts + self.transformer_weight * dec_out_selected

        combined_forecasts = self.fc(combined_forecasts)

        if self.output_attention:
            return combined_forecasts, attns
        else:
            return combined_forecasts  # [B, L, D]
